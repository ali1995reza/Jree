...
public class MongoDetailsStore<ID extends Comparable<ID>> implements DetailsStore<ID> {
    ...
    public MongoDetailsStore(AsyncMongoDatabase database, BatchContext batchContext) {
        ...
        this.clientsDetailsStoreCollection = this.database.getCollection(CLIENT_DETAILS_STORE_COLLECTION_NAME);
        this.relationStoreCollection = this.database.getCollection(RELATION_DETAILS_STORE_COLLECTION_NAME);
        this.sessionsDetailsStoreCollection = this.database.getCollection(SESSIONS_DETAILS_STORE_COLLECTION_NAME);
        this.subscribesCollection = this.database.getCollection(SUBSCRIBES_STORE_COLLECTION_NAME);
        this.batchContext.createNewUpdateBatch("clients", clientsDetailsStoreCollection, 2000, 100);
        this.batchContext.createNewUpsertBatch("clients", clientsDetailsStoreCollection, UpsertBatch.LONG_FETCHER, 2000, 100);
        this.batchContext.createNewFindBatch("clients", clientsDetailsStoreCollection, 1000, 100);
        this.batchContext.createNewFindBatch("sessions", sessionsDetailsStoreCollection, 1000, 100);
        this.batchContext.createNewUpsertBatch("sessions", sessionsDetailsStoreCollection, UpsertBatch.STRING_FETCHER, 1000, 100);
        this.batchContext.createNewUpdateBatch("relation", relationStoreCollection, 2000, 100);
        this.batchContext.createNewFindBatch("relation", relationStoreCollection, 1000, 50);
        this.batchContext.createNewUpsertBatch("subscribe", subscribesCollection, UpsertBatch.STRING_FETCHER, 1000, 100);
        DBStaticFunctions.createIndex(clientsDetailsStoreCollection, Indexes.ascending("client", "sessions"));
        DBStaticFunctions.createIndex(sessionsDetailsStoreCollection, Indexes.ascending("client", "session"));
    }

    private void doAddClient(long client, OperationResultListener<Boolean> callback) {
        batchContext.getUpsertBatch("clients").upsertOne(client, wrapCallBack(callback), UpsertBatch.SetValue.setValue("joinTime", Instant.now()));
    }

    public void doAddSessionToClient(long client, OperationResultListener<Long> callback) {
        batchContext.getFindBatch("clients").findOne(client, new SingleResultCallback<Document>() {
            @Override
            public void onResult(Document document, Throwable throwable) {
                if (throwable != null) {
                    ...
                    batchContext.getUpsertBatch("sessions").upsertOne(String.valueOf(client).concat("_").concat(String.valueOf(sessionId)), new SingleResultCallback<Boolean>() {
                        @Override
                        public void onResult(Boolean aBoolean, Throwable throwable) {
                            if (throwable != null) {
                                ...
                                if (aBoolean) {
                                    ...
                                    callback.onFailed(new FailReason(new IllegalStateException("fix it after"), FailReasonsCodes.RUNTIME_EXCEPTION));
                                }
                            }
                        }
                    }, UpsertBatch.SetValue.setValue("client", client), UpsertBatch.SetValue.setValue("session", sessionId), UpsertBatch.SetValue.setValue("offset", "0"));
                }
            }
        });
    }
    ...
    private static String uniqueId(long client, long session) {
        return String.valueOf(client).concat("_").concat(String.valueOf(session));
    }
    ...
    public void getRelation(String relationId, OperationResultListener<Relation> result) {
        batchContext.getFindBatch("relation").findOne(relationId, new SingleResultCallback<Document>() {
            @Override
            public void onResult(Document document, Throwable throwable) {
                if (throwable != null) {
                    result.onFailed(new FailReason(throwable, FailReasonsCodes.RUNTIME_EXCEPTION));
                    return;
                }
                if (document == null) {
    ...
    public void isSessionExists(long client, long session, OperationResultListener<Boolean> callback) {
        batchContext.getFindBatch("sessions").findOne(String.valueOf(client).concat("_").concat(String.valueOf(session)), new SingleResultCallback<Document>() {
            @Override
            public void onResult(Document document, Throwable throwable) {
                if (throwable != null) {
                    callback.onFailed(new FailReason(throwable, FailReasonsCodes.RUNTIME_EXCEPTION));
    ...
    public void getSessionOffset(long client, long session, OperationResultListener<ID> callback) {
        batchContext.getFindBatch("sessions").findOne(uniqueId(client, session), new SingleResultCallback<Document>() {
            @Override
            public void onResult(Document document, Throwable throwable) {
                if (throwable != null) {
                    callback.onFailed(new FailReason(throwable, FailReasonsCodes.RUNTIME_EXCEPTION));
        ...
        batchContext.getFindBatch("sessions").findOne(uniqueId(client, session), new SingleResultCallback<Document>() {
            ...
            public void onResult(Document document, Throwable throwable) {
                if (throwable != null) {
                    ...
                    details.setSessionExists(true).setOffset((ID) document.get("offset"));
                    List<Long> conversations = new ArrayList<>();
                    subscribesCollection.find(eq("client", client)).projection(Projections.include("conversation")).forEach(new Block<Document>() {
                        @Override
                        public void apply(Document document) {
                            conversations.add(document.getLong("conversation"));
                        }
                        ...
                        public void onResult(Void unused, Throwable throwable) {
                            if (throwable != null) {
                                ...
                                callback.onSuccess(details.setSubscribeList(conversations));
                            }
                        }
                    });
                }
    ...
    public void setSessionOffset(long client, long session, ID offset, OperationResultListener<Boolean> callback) {
        batchContext.getUpdateBatch("sessions").updateOne(and(eq("client", client), eq("session", session)), set("offset", offset), new UpdateOptions().upsert(true), alwaysTrueCallback(callback));
    }
    ...
    public void addRelation(Session session, Recipient recipient, String key, String value, OperationResultListener<Boolean> callback) {
        ...
        batchContext.getUpdateBatch("relation").updateOne(eq("_id", id), set("CL_".concat(String.valueOf(session.clientId())).concat(".").concat(key), value), new UpdateOptions().upsert(true), alwaysTrueCallback(callback));
    }

    @Override
    public void getRelation(Session session, Recipient recipient, OperationResultListener<Relation> callback) {
        if (recipient.conversation() < 0) {
            getRelation(StaticFunctions.uniqueConversationId(session, recipient), callback);
        } else {
    ...
    public void addToSubscribeList(long client, long conversation, OperationResultListener<Boolean> callback) {
        batchContext.getUpsertBatch("subscribe").upsertOne(String.valueOf(client).concat("_").concat(String.valueOf(conversation)), new SingleResultCallback<Boolean>() {
            @Override
            public void onResult(Boolean aBoolean, Throwable throwable) {
                if (throwable != null) {
                    callback.onFailed(new FailReason(throwable, FailReasonsCodes.RUNTIME_EXCEPTION));
        ...
        }, UpsertBatch.SetValue.setValue("client", client), UpsertBatch.SetValue.setValue("conversation", conversation));
    }

    @Override
    public void removeFromSubscribeList(long client, long conversation, OperationResultListener<Boolean> callback) {
        subscribesCollection.deleteOne(eq("_id", String.valueOf(client).concat("_").concat(String.valueOf(conversation))), new SingleResultCallback<DeleteResult>() {
            @Override
            public void onResult(DeleteResult deleteResult, Throwable throwable) {
                if (throwable != null) {
                    callback.onFailed(new FailReason(throwable, FailReasonsCodes.RUNTIME_EXCEPTION));
                } else {
                    callback.onSuccess(deleteResult.getDeletedCount() > 0);
                }
            }
        });
    }
